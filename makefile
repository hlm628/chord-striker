...
RUN ?= $(if $(filter true,$(IS_DOCKER)),,docker run --platform $(DOCKER_PLATFORM) --rm -v $(PWD):/app -w /app $(DOCKER_IMG):$(RUN_TAG))
...
run-example-song:
	make run-song SONG_NAME="Example" SEED=7 OUTPUT_DIR=example PRINT_GRAPH=True
